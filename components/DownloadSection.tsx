...
export default function DownloadSection() {
  ...
  const installationSteps = {
    windows: {
      ...
      commands: [
        ...
        'unzip local-ai-chat-windows.zip',
        'cd local-ai-chat-windows',
        '',
    ...
    mac: {
      ...
      extract: 'Mount the .dmg and copy the application to your Applications folder or preferred location',
    ...
    linux: {
      ...
      commands: [
        '# Extract the zip file',
        'unzip local-ai-chat-linux.zip',
        'cd local-ai-chat-linux',
        '',
        '# Install Ollama (if not already installed)',
  ...
  const handleDownloadZip = () => {
    ...
    a.href = link.file
    a.download = `local-ai-chat-${selectedOS}${link.extension}`
    ...
    <div className="w-full max-w-5xl mx-auto space-y-8 md:space-y-12 pb-12 md:pb-20 px-4 md:px-0" data-section="getting-started">
      ...
      {selectedOS && steps && (
        ...
        >
          ...
          >
            <button
              onClick={() => handleCopyCommand(steps.commands.filter(cmd => !cmd.startsWith('#') && cmd.trim()).join('\n'))}
              className="absolute top-2 right-2 p-1.5 hover:bg-[rgb(237,254,193)]/20 transition-colors rounded"
              style={{ borderRadius: '0px' }}
              title="Copy commands"
            >
              {copiedCommand === steps.commands.filter(cmd => !cmd.startsWith('#') && cmd.trim()).join('\n') ? (
                <Check className="w-4 h-4 text-[rgb(237,254,193)]" />
              ) : (
                <Copy className="w-4 h-4 text-[rgb(237,254,193)]" />
              )}
